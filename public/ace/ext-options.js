...
define('ace/ext/options', ['require', 'exports', 'module'], function (require, exports, module) {
	...
	var options = [["Document", function (name) {
	...
	}, doclist.all], ["Mode", function (value) {
	...
	}, modelist.modes], ["Split", function (value) {
		var sp = env.split;
		if (value == "none") {
			if (sp.getSplits() == 2) {
	...
	}, ["None", "Beside", "Below"]], ["Theme", function (value) {
		if (!value) return;
	...
	}, ["manual", "mark begin", "mark begin and end"]], ["Soft Wrap", function (value) {
	...
	}, ["Off", "40 Chars", "80 Chars", "Free"]], ["Key Binding", function (value) {
		env.editor.setKeyboardHandler(keybindings[value]);
	}, ["Ace", "Vim", "Emacs", "Custom"]], ["Font Size", function (value) {
		env.split.setFontSize(value + "px");
	}, [10, 11, 12, 14, 16, 20, 24]], ["Full Line Selection", function (checked) {
	...
	var createOptionsPanel = function createOptionsPanel(options) {
		var html = [];
		...
		html.push('<tr>', '<td>', '<label for="', s, '"></label>', '</td><td>', '<input type="', s, '" name="', s, '" id="', s, '">', '</td>', '</tr>');
	...
	function bindCheckbox(id, callback) {
		...
		if (localStorage && localStorage.getItem(id)) el.checked = localStorage.getItem(id) == "1";
	...
	function bindDropdown(id, callback) {
		var el = document.getElementById(id);
		if (localStorage && localStorage.getItem(id)) el.value = localStorage.getItem(id);
